...
public interface FlowerRepository extends JpaRepository<Flower, Long> {
    ...
    @Query(value = "SELECT f.id, f.name, f.description, f.category_id, f.created_at, f.stock_quantity, f.unit_price, f.url_image, f.updated_at,  (SUM(i.quantity) - f.stock_quantity) AS mostSale\n" +
            "            FROM flowers f\n" +
            "            JOIN imports i ON f.id = i.flower_id\n" +
            "            GROUP BY f.id, f.name\n" +
            "            ORDER BY mostSale DESC\n" +
            "            LIMIT 10;",nativeQuery = true)
    List<Object[]> findByBestSeller();
